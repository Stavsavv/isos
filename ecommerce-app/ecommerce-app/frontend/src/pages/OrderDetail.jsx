        ...
        <div className="card p-6">
            ...
            {order.items?.map((item, i) => (
              <div key={i} className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-surface-500 text-xs">Qty: {item.quantity} x {formatCurrency(item.price)}</p>
                  {item.shotNumber ? (
                    <p className="text-surface-500 text-xs">Shot number: {item.shotNumber}</p>
                  ) : null}
                </div>
                <span className="font-semibold text-sm">{formatCurrency((item.price || 0) * (item.quantity || 0))}</span>
              </div>
            ))}
          </div>
          <div className="border-t border-surface-200 dark:border-surface-700 mt-4 pt-4 space-y-1 text-sm">
            <div className="flex justify-between font-bold text-base">
              <span>Total</span>
              <span className="text-primary-500">{formatCurrency(order.totalPrice)}</span>
            </div>
          </div>
        </div>
